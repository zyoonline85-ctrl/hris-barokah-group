/**
 * ProgramPelatihanBaru.jsx — Modul Program Pelatihan & Pengembangan Kompetensi
 * ===========================================================================
 * HRIS Barokah Grup — Sistem Tiga Tab Otonom
 * Kompatibel dengan Tema Gelap (Cyber Tech) dan Terang (Sage Green)
 */
...
const Btn = ({ children, onClick, variant='primary', disabled=false, style={}, type='button', id, className }) => {
  const v = {
  ...
  return (
    <button id={id} className={className} type={type} onClick={onClick} disabled={disabled} style={{
...
// MAIN COMPONENT
// ═══════════════════════════════════════════════════════════════════════════════
export default function ProgramPelatihanBaru({ token, API_URL }) {
  ...
  // Results & materials — filter by bulan/tahun/outlet/jabatan
  ...
  const handleSaveScore = useCallback(({ attendance, score, is_lulus }) => {
    ...
    // ── KPI Sync: tulis ke hris_training_kpi_scores (dibaca KpiPage & Payroll)
  ...
  // 1. Export Hanya Jadwal Training
  const exportSchedulePDF = () => {
    ...
    setTimeout(() => {
      try {
        ...
        doc.text('LAPORAN JADWAL AGENDA TRAINING BAROKAH GRUP', 14, 14);
        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
        doc.setTextColor(180, 180, 180);
        doc.text(`Jadwal Agenda Training · Dicetak: ${new Date().toLocaleString('id-ID')}`, 14, 22);

        const headers = [['Tanggal & Jam', 'Nama Pelatihan', 'Target Outlet', 'Target Jabatan', 'Mentor', 'Status']];
        ...
        doc.save(`Laporan_Jadwal_Pelatihan_${new Date().toISOString().slice(0, 10)}.pdf`);
      } catch (err) {
  ...
  const exportResultsPDF = () => {
    ...
    setTimeout(() => {
      try {
        ...
        doc.setFontSize(16);
        doc.setFont('helvetica', 'bold');
        doc.text('LAPORAN EVALUASI & HASIL TRAINING BAROKAH GRUP', 14, 14);
        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
        doc.setTextColor(180, 180, 180);
        doc.text(`Nilai Evaluasi Hasil Kelulusan · Dicetak: ${new Date().toLocaleString('id-ID')}`, 14, 22);

        const headers = [['Nama Karyawan', 'Outlet', 'Nama Pelatihan', 'Kehadiran (%)', 'Skor Akhir', 'Status Kelulusan']];
        const rows = filteredResults.map(r => [
          ...
          r.is_lulus === null ? 'Belum Dinilai' : r.is_lulus ? 'Lulus' : 'Tidak Lulus'
        ]);
        ...
        doc.save(`Laporan_Hasil_Evaluasi_${new Date().toISOString().slice(0, 10)}.pdf`);
      } catch (err) {
        console.error('PDF export failed:', err);
      } finally {
        setIsExportingPDF(false);
  ...
  const handleExportPDF = () => {
    ...
    setTimeout(() => {
      try {
        ...
        doc.setFontSize(16);
        doc.setFont('helvetica', 'bold');
        doc.text('LAPORAN PROGRAM PELATIHAN BAROKAH GRUP', 14, 14);
        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
        doc.setTextColor(180, 180, 180);
        doc.text(`Bagian 1: Jadwal Agenda Training · Dicetak: ${new Date().toLocaleString('id-ID')}`, 14, 22);

        const scheduleHeaders = [['Tanggal & Jam', 'Nama Pelatihan', 'Target Outlet', 'Target Jabatan', 'Mentor', 'Status']];
        ...
        doc.setFont('helvetica', 'normal');
        doc.setTextColor(180, 180, 180);
        doc.text(`Bagian 2: Nilai Evaluasi Hasil Kelulusan · Total: ${filteredResults.length} Karyawan`, 14, 22);

        const resultHeaders = [['Nama Karyawan', 'Outlet', 'Nama Pelatihan', 'Kehadiran (%)', 'Skor Akhir', 'Status Kelulusan']];
        ...
        doc.setFont('helvetica', 'normal');
        doc.setTextColor(180, 180, 180);
        doc.text(`Bagian 3: Daftar Modul Materi Pelatihan · Total: ${filteredMaterials.length} Modul`, 14, 22);

        const materialHeaders = [['Judul Modul', 'Deskripsi', 'Tipe Dokumen', 'Nama File']];
  ...
  return (
    <div style={{ fontFamily:"'Inter','Segoe UI',sans-serif", background:C.bg, minHeight:'100vh', color:C.text }}>
      ...
      <div style={{ background:`linear-gradient(135deg,${C.surface} 0%,#252B32 100%)`, borderBottom:`1px solid ${C.border}`, padding:'26px 32px 20px', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ marginTop:'20px', display:'grid', gridTemplateColumns:'1fr 1fr auto auto auto', gap:'12px', alignItems:'end' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'5px' }}>
            ...
            <Btn id="global-pdf-btn" className="pdf-btn-black" onClick={handleExportPDF} style={{ height:'40px' }}>
          ...
          <div className="tab-anim">
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'18px', flexWrap:'wrap', gap:'12px' }}>
              ...
              <div style={{ display:'flex', gap:'10px' }}>
                <Btn id="schedule-pdf-btn" className="pdf-btn-black" onClick={exportSchedulePDF} style={{ height:'40px' }}><Download size={14}/> Download PDF</Btn>
        ...
        {activeTab === 'hasil' && (
          <div className="tab-anim">
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'18px', flexWrap:'wrap', gap:'12px' }}>
              ...
              <Btn id="results-pdf-btn" className="pdf-btn-black" onClick={exportResultsPDF} style={{ height:'40px' }}><Download size={14}/> Download PDF</Btn>
            </div>
                        ...
                        <td style={{ padding:'14px 16px' }}>
                          {r.is_lulus === null
                            ...
                            : r.is_lulus
                              ...
                              : <Badge label="❌ Remedial" color={C.danger} bg={C.dangerDim}/>}
                        </td>
                        <td style={{ padding:'14px 16px' }}>
                  ...
                  return (
                    <div key={m.id} className="mat-card" style={{ background:C.surface, borderRadius:'14px', border:`1px solid ${C.border}`, padding:'20px', cursor:'default', display:'flex', flexDirection:'column' }}>
                      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'12px' }}>
                        <div style={{ background:`${typeColor[m.file_type]}18`, border:`1px solid ${typeColor[m.file_type]}33`, borderRadius:'10px', padding:'10px', display:'flex', alignItems:'center', justifyContext:'center' }}>
      ...
      {/* Marching Ants Cyan Dotted Line Animation for Live Sending */}
      {isSendingEvent && (
        <div style={{
          position: 'fixed',
          ...
          {/* Cyan Dotted Line Animation - 0.2s Marching Ants */}
          <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '4px' }}>
            <svg width="100%" height="4" style={{ display: 'block', overflow: 'visible' }}>
              <line x1="0" y1="2" x2="100%" y2="2" stroke="#00ADB5" strokeWidth="4" strokeDasharray="20 12" style={{ animation: 'marchingAnts 0.2s linear infinite' }} />
            </svg>
          </div>
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '4px' }}>
            <svg width="100%" height="4" style={{ display: 'block', overflow: 'visible' }}>
              <line x1="0" y1="2" x2="100%" y2="2" stroke="#00ADB5" strokeWidth="4" strokeDasharray="20 12" style={{ animation: 'marchingAnts 0.2s linear infinite reverse' }} />
            </svg>
          </div>
          <div style={{
            ...
            <span style={{ color: '#00ADB5', fontWeight: 'bold' }}>
              Menerbitkan Jadwal Training & Mengirimkan Undangan Seluler...
            </span>
          </div>
          <style>{`
